...
import { getSuperAdminThread, getSuperAdminPartner, sendSuperAdminMessage } from '../_actions'

export default async function SuperAdminThreadPage({ params }: { params: Promise<{ partnerId: string }> }) {
  const { partnerId } = await params
  const session = await getServerSession(authOptions)
  if (!session?.user?.id) return null

  const [messages, partner] = await Promise.all([
    getSuperAdminThread(partnerId),
    getSuperAdminPartner(partnerId),
  ])
  ...
  async function handleSend(data: { toId: string; text: string }) {
    'use server'
    return sendSuperAdminMessage(data)
  }

  return (
    <div className="flex flex-col flex-1 sm:h-[calc(100vh-8rem)] max-w-2xl space-y-4">
      <div className="flex items-center gap-3">
        <Button render={<Link href="/superadmin/messages" />} variant="ghost" size="sm">
